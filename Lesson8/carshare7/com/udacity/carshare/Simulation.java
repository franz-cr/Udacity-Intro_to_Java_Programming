...
public class Simulation
{
    ...
    */
    ...
    {
        ArrayList<Person> dropOffPassengers = new ArrayList<Person>();
        
        while (cars.size() > 0) {
            int i = 0;
            while (i < cars.size()) {
                Car c = cars.get(i);
                dropOffPassengers = c.drive();
                // TODO: Add print statement here
                for (int j = 0; j < dropOffPassengers.size(); j++)
                    System.out.println(c.getDriverName() + " drops off " + 
                        dropOffPassengers.get(j).getName());
